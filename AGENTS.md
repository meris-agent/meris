...
| 排熵扫描 / handoff | `meris harness gc` · `.meris/handoff.md` |
| 提交前校验 | `meris harness commit-check` · `bash scripts/install-git-hooks.sh` |
...
- 根目录文档：`README.md`、`AGENTS.md`；跨会话进度写在各项目根 `PROGRESS.md`
...
- 不要自动 `git push`  
- 不要删已有 migration  
- 不要提交 `.env`、API key、`.meris/sessions/`、Cloud 部署密钥 — 见 `meris harness commit-check`

## 会话约定

- 新会话：改 Meris 源码 → 读 `docs/` 与 `AGENTS.md`；在用户项目上 → 读该项目根目录 `PROGRESS.md`
...
meris harness check
meris harness commit-check --tracked   # CI；本地提交前由 git hook 自动跑 --cached
```
...
首次 clone 后安装 git hook（每次 commit 自动扫描暂存区）：
...
```bash
meris harness commit-check --cached    # 暂存区
meris harness commit-check --tracked   # 全仓库 tracked 文件（CI 同款）
```

拦截项包括：`.env`（非 example）、`.meris/sessions/`、常见 API token 格式等。团队可扩展 `.meris/commit-guard.yaml`（见 `templates/commit-guard.example.yaml`）。详见 `meris/harness/commit_guard.py`。
